...
TEST_F(MultipleEvaluationTest, LegacyEvaluateMultipleNodesCorrectValues) {
    auto results = eval.EvaluateGraph({c2, c3, c4});
    ...
    results = eval.EvaluateGraph(nodes);
    EXPECT_FLOAT_EQ(results[c2->Channels(0)].ToScalar(), 2.0);
    EXPECT_FLOAT_EQ(results[c3->Channels(0)].ToScalar(), 3.0);
    EXPECT_FLOAT_EQ(results[c4->Channels(0)].ToScalar(), 4.0);
}

TEST_F(MultipleEvaluationTest, LegacyEvaluateMultipleInputsReturnsOverrides) {
    vars[x] = Scalar(1.0);
    vars[y] = Scalar(2.0);
    vars[z] = Scalar(3.0);
    auto results = eval.EvaluateGraph({x, y, z}, vars);
...
TEST_F(MultipleEvaluationTest, LegacyFunctionWithSharedComponentsReturnsCorrectOutputs) {
    vars[x] = Scalar(1.0);
    vars[y] = Scalar(2.0);
    vars[z] = Scalar(3.0);
    auto results = eval.EvaluateGraph({plusFunction, minusFunctionPlusTwo}, vars);
...
TEST_F(MultipleEvaluationTest, LegacyRepeatedNodeInEvaluationRequestReturnsCorrectOutputs) {
    vars[x] = Scalar(1.0);
    vars[y] = Scalar(2.0);
    vars[z] = Scalar(3.0);
    auto results = eval.EvaluateGraph({plusFunction, plusFunction, plusFunction, minusFunctionPlusTwo}, vars);
    float left = 1.0 * 2.0 - 3.0;
    float right = 2.0 * 3.0 + 4.0;
    EXPECT_FLOAT_EQ(results[plusFunction->Channels(0)].ToScalar(), left + right);
    EXPECT_FLOAT_EQ(results[minusFunctionPlusTwo->Channels(0)].ToScalar(), left - right + 2.0);
